...
function App({ doggys, setFilter }) {
  const changeFilter = (e) => {
    ...
    switch (select) {
      ...
      case "weigth+":
        const pesados = array.sort((a, b) =>
          Number(spliter(a.weight)) > Number(spliter(b.weight)) ? -1 : 1
        );
        return setFilter(pesados);
      case "weigth-":
        const livianos = array.sort((a, b) =>
          Number(spliter(a.weight)) > Number(spliter(b.weight)) ? 1 : -1
        );
        return setFilter(livianos);
      case "byYou":
        const byYou = array.filter((e) => {
          return e.id > 264;
        });
        return setFilter(byYou);
      case "Api":
        const api = array.filter((e) => {
          return e.id < 264;
        });
  ...
  return (
    <div className="App">
      ...
      <Route exact path="/dogs/add" component={Form} />
